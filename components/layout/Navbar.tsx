...
const Navbar: React.FC<NavbarProps> = ({ userEmail, onLogout, onLoginRequest, onBrandClick, sidebarOpen, onToggleSidebar }) => {
  ...
  return (
    <>
    <nav className={`fixed top-0 left-0 right-0 h-14 flex items-center justify-between px-4 ${sidebarOpen ? 'md:pl-[240px]' : 'pl-4'} bg-gradient-to-r from-slate-900/95 via-slate-800/95 to-slate-900/95 dark:from-gray-900/95 dark:via-gray-800/95 dark:to-gray-900/95 backdrop-blur-lg border-b border-slate-600/30 dark:border-gray-600/30 z-40 navbar shadow-lg transition-all duration-300 ease-in-out`}>
      <div className="flex items-center gap-3">
        {/* Sidebar Toggle Button */}
        <button
          onClick={onToggleSidebar}
          className="w-9 h-9 rounded-lg flex items-center justify-center text-slate-400 hover:text-white hover:bg-slate-700/50 transition-colors duration-200 border border-slate-600/30 hover:border-slate-500/50"
        ...
        {/* Logo & Brand */}
        <div className="flex items-center gap-3 cursor-pointer select-none" onClick={onBrandClick} title="Support HR">
                ...
                <div className="py-1">
                  ...
                  <div className="px-4 py-2 border-b border-gray-200 dark:border-gray-700">
                    ...
                    <button
                      ...
                      className="w-full flex items-center px-2 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md transition-colors duration-150"
                    >
                      <i className={`fa-solid ${isDarkMode ? 'fa-sun' : 'fa-moon'} w-4 text-center mr-3 text-gray-500`}></i>
                      <span className="flex-1 text-left">{isDarkMode ? 'Chế độ sáng' : 'Chế độ tối'}</span>
                      <div className={`w-8 h-4 rounded-full transition-colors duration-200 ${isDarkMode ? 'bg-blue-500' : 'bg-gray-300'} relative`}>
                        <div className={`w-3 h-3 rounded-full bg-white absolute top-0.5 transition-transform duration-200 ${isDarkMode ? 'translate-x-4' : 'translate-x-0.5'}`}></div>
                      </div>
                    </button>
                    ...
                    <button
                      ...
                      className="w-full flex items-center px-2 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md transition-colors duration-150"
                    >
                      <i className={`fa-solid ${sidebarOpen ? 'fa-eye' : 'fa-eye-slash'} w-4 text-center mr-3 text-gray-500`}></i>
                      <span className="flex-1 text-left">{sidebarOpen ? 'Ẩn thanh bên' : 'Hiện thanh bên'}</span>
                      <div className={`w-8 h-4 rounded-full transition-colors duration-200 ${sidebarOpen ? 'bg-blue-500' : 'bg-gray-300'} relative`}>
                        <div className={`w-3 h-3 rounded-full bg-white absolute top-0.5 transition-transform duration-200 ${sidebarOpen ? 'translate-x-4' : 'translate-x-0.5'}`}></div>
                      </div>
                    </button>

                    {/* Avatar Upload */}
                    <label className="w-full flex items-center px-2 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md transition-colors duration-150 cursor-pointer">
                      <i className="fa-solid fa-camera w-4 text-center mr-3 text-gray-500"></i>
