...
import { evaluateInstitutionsFromEducation } from './institutionsData';
...
export function generateDeterministicScore(input: DeterministicInput): DeterministicResult {
	...
	return {
		extracted: {
			...
			roles,
			education,
			recognized_institutions: instEval.matches.map(m => ({ name: m.matched?.name, tier: m.matched?.tier, weight: m.matched?.qualityWeight })),
